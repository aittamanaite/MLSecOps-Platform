...
│   ├── models.py        # Pydantic schemas (FlowItem, BatchPredictRequest)
...
## 13. Module reference (read the folder READMEs)

This repository ships focused `README.md` files alongside the code so you
can understand each part without reading every source file. Start with the
one(s) relevant to your task:

- **`app/`** — the HTTP API and its Pydantic request schemas.
  See [`app/readme.md`](app/readme.md) for every endpoint
  (`/health`, `/predict`, `/predict/batch`, `/metrics`), the exact request
  schema fields that must be sent, and how to open & use **Prometheus and
  Grafana**.

- **`src/ml/`** — the ML pipeline (feature engineering, dataset
  preparation, training, and inference).
  See [`src/ml/readme.md`](src/ml/readme.md) for each module and how to
  run training and prediction manually.
