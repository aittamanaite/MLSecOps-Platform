...
# ==========================================
@sensor(
    name="csv_streaming_sensor", job=streaming_pipeline_job, minimum_interval_seconds=30
)
def csv_streaming_sensor(context: SensorEvaluationContext):
    ...
    for filepath in csv_files:
        try:
            ...
            context.log.warning(
                f"Impossible d'accéder aux statistiques de {filepath} : {e}"
            )
    ...
    if current_state != last_state:
        ...
        context.log.info(
            "Nouveaux fichiers locaux détectés dans data/raw/. Lancement du traitement Streaming..."
        )
...
# ==========================================
defs = Definitions(
    assets=[streaming_ingestion_asset, streaming_cleaning_asset, model_inference_asset],
