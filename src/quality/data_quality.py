...
def validate_raw_csv(df: pd.DataFrame) -> QualityReport:
    ...
    missing_critical = [c for c in CRITICAL_COLUMNS_FOR_PIPELINE if c not in df.columns]
    report.add_result(
        RuleResult(
    ...
    if "Label" in df.columns:
        ...
        report.add_result(
            RuleResult(
                ...
                details={
                    "null_rate": round(label_null_rate, 6),
    ...
    if "Destination Port" in df.columns:
        ...
        report.add_result(
            RuleResult(
                ...
                details={
                    "null_rate": round(dp_null_rate, 6),
                    "threshold": MAX_NULL_RATE,
                },
            )
        )
    ...
    if "Destination Port" in df.columns:
        ...
        report.add_result(
            RuleResult(
                ...
                message=f"All ports in valid range [0, 65535]"
                if total_invalid == 0
    ...
    if "Label" in df.columns:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All labels are valid CICIDS2017 categories"
    ...
    report.add_result(
        RuleResult(
            ...
            message="No infinite values in numeric columns"
            if inf_count == 0
    ...
    report.add_result(
        RuleResult(
            ...
            category="integrity",
            passed=schema_match,
            message="Column names match expected CICIDS2017 schema"
            if schema_match
...
def validate_cleaned_records(records: list[dict]) -> QualityReport:
    ...
    report.add_result(
        RuleResult(
            ...
            category="completeness",
            passed=total > 0,
            message=f"{total} cleaned records"
            if total > 0
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All critical keys present in every record"
                if records_with_missing == 0
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All keys are properly normalized (snake_case)"
                if not bad_keys_set
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All destination ports in valid range"
    ...
    if total > 0:
        ...
        for rec in records:
            ...
            if (
                lbl is not None
                and str(lbl).strip() not in VALID_LABELS
                and str(lbl).strip().lower() not in VALID_LABELS
                and lbl != 0
            ):
        ...
        report.add_result(
            RuleResult(
                ...
                message="All cleaned label values are valid"
                if not unknown_unique
...
def validate_inference_output(
    records: list[dict], total_processed: int = 0
) -> QualityReport:
    """
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All anomaly records have ML metadata"
                if missing_meta == 0
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All confidence scores in [0.0, 1.0]"
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message="All records have valid model version strings"
    ...
    if total > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                message=f"All anomaly records exceed threshold {ANOMALY_THRESHOLD}"
    ...
    if total_processed > 0:
        ...
        report.add_result(
            RuleResult(
                ...
                category="integrity",
                passed=plausible,
                message=f"{total} anomalies out of {total_processed} processed — plausible"
                if plausible
